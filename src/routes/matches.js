...
const MAX_LIMIT = 100; // Maximum limit for pagination, can be adjusted as needed
...
matchRouter.get("/", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({success: true, message: "Matches retrieved successfully", data: matchesList });
  } catch (error) {
    res.status(500).json({ error: 'Failed to retrieve matches.', details: JSON.stringify(error) });
  }
});
